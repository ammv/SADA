...
{
    public class PurchaseFromCounteragentViewModel : TabObservableObjectForm<CarPaymentFromCounteragent>
    {
        #region Fields
        ...
        public PurchaseFromCounteragentViewModel(IDialogService dialogService, ITabService tabService)
        {
        ...
        private void _FormCommand()
        {
            try
            {
                ...
                {
                    string msg = "Запись об оплате от контрагента за автомобиль обновлена";
                    if (_currentFormMode == FormMode.Add)
                    {
                        _ctx.CarPaymentFromCounteragent.Add(Entity);
                        msg = "Новая запись об оплате от контрагента за автомобиль добавлена";
                    }
                    ...
                    if (_currentFormMode == FormMode.Add)
                    {
                        Entity = new CarPaymentFromCounteragent();
        ...
        private void _loadedInner()
        {
            ...
            if (_currentFormMode == FormMode.Edit || _currentFormMode == FormMode.See)
            {
                ...
                Entity = _ctx.CarPaymentFromCounteragent
